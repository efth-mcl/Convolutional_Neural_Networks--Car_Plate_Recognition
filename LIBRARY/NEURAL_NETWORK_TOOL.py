...
class NNtool:
    ...
    def buildnet(self,TopologyDir,NewWeights=True):
            ...
            if os.system('ls ../TOPOLOGYS/'+TopologyDir+'/*.Bin')==512: #512 -> False -> Not Find Weigts In Topology Directory
                self.NewWeights=True
                print('Not Find Weight and Bias .Bin files.')
                print('New Weight and Bias .Bin files.')
            else:
                if self.NewWeights==True:
    ...
    def ReadFile(self):
        ...
        with open(self.topologytxtname,"r") as fl:
            ...
            self.Idropout=0 # keeps from Topology the previous Layer's index from the last Dropout layer
            self.DroupoutCnt=0
            ...
            if(Str[0][:Str[0].index('(')]!='Input'): #if the  topology.txt does't exist, start with Input Layer
        ...
        self.y_= tf.placeholder(tf.float32, shape=[None, self.LayerShape(self.Layers[-1])[-1]]) #Here placed the true classes

    def Case(self,Stri):
        layertype = Stri[:Stri.index('(')]

        layerparams=Stri[Stri.index('(')+1:-1].split(',')
        layerparams[0]=int(layerparams[0]) # layerparams examples : [2,'2','1'] or [4] or [9,32,'Sigmoid']
    ...
    ##START TRAIN ###############################################################
    def TRAIN(self,Epochs,BatchSize,Te=1,test_predict=True,Tb=40,trainrate=1e-4,TrPrTb=True,TsPrTb=True): # Te           :Train frequency measurement
        if self.Test_Data==None or self.Test_Hot==None :                                                  # Tb           :Train batch measurement
            test_predict=False                                                                            # TrPrTb       :boolean  for Train Prediction Table
            TsPrTb=False                                                                                  # test_predict :boolean for Test
        if Te==0:
            test_predict=False
    ...
    # Loss and accuracy are calculated per 1000 samples, because of memory limitation
...
####START VISUALIZATION
    ...
    def TrainTestPlot(self):
        if 'train' in self.DictData and 'test' in self.DictData:
            ...
            plt.legend()
            plt.show()
    # print three at most plots with Train Loss-Accuracy, Test Loss-Accuracy, Batch Loss-Accuracy
