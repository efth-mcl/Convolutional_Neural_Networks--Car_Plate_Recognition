...
def HE(img,T0=70,T1=170,NT0=170,NT1=255,Dark0=5,Dark1=50):
    ...
    plt.fill_between(L, 0, hist.max(),
        where=abs(L-(Dark1+Dark0)/2)<=(Dark1-Dark0)/2,
        facecolor='#178D36',
        alpha=0.7,
        label=r'$Sum_{hist_{i}}\geqslant S_{dark}$'
        )
    plt.plot(np.array([Dark0,Dark0]),np.array([0,hist.max()]),'--',color='#178D36')
    plt.plot(np.array([Dark1,Dark1]),np.array([0,hist.max()]),'--',color='#178D36')
    plt.legend()
...
def Cany_skimage(img,s=3,Tw=-1,Ts=-1):
    ...
    thita=np.arctan2(FL.sobel_v(img),FL.sobel_h(img)) #find continuous angle
    Ta=np.zeros((thita.shape[0],thita.shape[1])) #zero array with thita's shape
    ...
    #2) per cluster find and return the corner points of rectangle that is contained
def DBs(Pon):
    import numpy as np
    from sklearn.cluster import DBSCAN
    import matplotlib.pyplot as plt
    Rect=[]
    Eps=np.sqrt(2)+0.001 #All coordinates are intergers and we need square area around the points
    Min_samples=2
...
# heightCluster return Rect Clusters which have almost the same posx0a & posx1, that means same height and position
def heightCluster(Rect,Esp=15,Min_samples=4):
    ...
    while Rect.shape[0]>0:
        A=np.sqrt((poni[K[0]]-Rect[:,K[0]])**2+(poni[K[1]]-Rect[:,K[1]])**2)<=Esp #Euclidean distance
        if np.count_nonzero(A)>=Min_samples:
            ...
            while PointsToNext.shape[0] > 0:
                ...
                A=np.sqrt((Pi[K[0]]-Rect[:,K[0]])**2+(Pi[K[1]]-Rect[:,K[1]])**2)<=Esp
                if np.count_nonzero(A)>=Min_samples:
                    A=np.where(A==True)
                    Np=Rect[A]
    ...
    for i in range(len(Rect)):
        ...
        if (posx1-posx0)*(posy1-posy0)<A and (posx1-posx0)*(posy1-posy0)>0.05*A and (posx1-posx0)>H:
            ...
            while Bool and j<len(Rect2):
                if (posy0-Rect2[j][1])*(posy1-Rect2[j][3])<=0 and (posx0-Rect2[j][0])*(posx1-Rect2[j][2])<=0: #overlap operation
                    Bool=False
...
def RectsDrawing(ImP,Rect,c):
    ...
    for I in range(0,n_clusters_):
        ...
        pos1x=Rect[I][2]
        pos1y=Rect[I][3]
        plt.plot([pos0y,pos1y,pos1y,pos0y,pos0y],[pos0x,pos0x,pos1x,pos1x,pos0x],'--b')
    plt.imshow(ImP)
